...
const menuItems = [
  ...
  { name: "Log Out", icon: <Ionicons name="log-out-outline" size={24} color="#EF4444" /> },
];
...
const NavItems: React.FC<NavItemsProps> = ({ activeItem, setActiveItem }) => {
  return (
    <View className="flex-1">
      ...
      <View className="flex-1">
        {menuItems.map((item, index) => {
          ...
          return (
            ...
            >
              <View style={{ width: 35, alignItems: "center" }}>{item.icon}</View>
              <Text
      ...
      <View className="flex-row justify-center items-center gap-2">
        <Image
          ...
          resizeMode="contain"
        />
        <Text className="text-sm font-bold text-[#4392F9] font-libre">Stylish</Text>
      </View>
    </View>
  );
};
